...
fn missing_yaml_metadata_from_value(value: &Value) -> Vec<String> {
    let mapping = match value {
        ...
        _ => return DEFAULT_YAML_METADATA_KEYS.iter().map(|key| (*key).to_string()).collect(),
    };
...
#[cfg(test)]
mod tests {
    use super::{is_key_markdown_doc, missing_markdown_metadata, missing_yaml_metadata};
...
"#;
        ...
        assert_eq!(missing_markdown_metadata(text), vec!["lastReviewedCommit".to_string()]);
    }
